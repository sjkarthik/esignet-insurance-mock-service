...
function installing_mock-relying-party-service() {
  ...
  DEFAULT_ESIGNET_SERVICE_URL='http://esignet-insurance.esignet-insurance/v1/esignet'
  read -p "Please provide Esignet service url : ( default: http://esignet-insurance.esignet-insurance/v1/esignet )" USER_PROVIDED_ESIGNET_SERVICE_URL
  ...
  helm -n $NS install mock-relying-party-service-insurance mosip/mock-relying-party-service \
  ...
  kubectl -n $NS get deploy mock-relying-party-service-insurance -o name |  xargs -n1 -t  kubectl -n $NS rollout status
